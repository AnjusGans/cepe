/**
   ...
   * Follow https://github.com/kenndev
*/
...
async function startkenn() {
    ...
    kenn.ev.on('messages.upsert', async chatUpdate => {
        ...
        require("./main.js")(kenn, m, chatUpdate, store)
        } catch (err) {
            console.log(err)
        }
    })
...
let Welcom = await getBuffer('https://malesin.xyz/welcome2?username=${num.split("@")[0]}&groupname=${metadata.subject}&membercount=${participants}&profile=${ppuser}&background=https://telegra.ph/file/1209827e2f2089f20c4c0.jpg')
                // Get Profile Picture Group
                try {
                ...
                if (anu.action == 'add') {
                    kenn.sendMessage(anu.id, { image: { url: ppuser }, contextInfo: { mentionedJid: [num] }, caption: `Welcome To ${metadata.subject} @${num.split("@")[0]}` })
                } else if (anu.action == 'remove') {
                    kenn.sendMessage(anu.id, { image: { url: ppuser }, contextInfo: { mentionedJid: [num] }, caption: `@${num.split("@")[0]} Leaving To ${metadata.subject}` })
                } else if (anu.action == 'promote') {
                    kenn.sendMessage(anu.id, { image: { url: ppuser }, mentions: [num], caption: `@${num.split('@')[0]} Promote From ${metadata.subject}` })
                } else if (anu.action == 'demote') {
                    kenn.sendMessage(anu.id, { image: { url: ppuser }, mentions: [num], caption: `@${num.split('@')[0]} Demote From ${metadata.subject}` })
              }
            }
        } catch (err) {
            console.log(err)
   ...
   /* kenn.ev.on('connection.update', async (update) => {
        const { connection, lastDisconnect } = update	    
        if (connection === 'close') { console.log("KurangTw"); startkenn(); }
    })*/
    ...
    // Add Other
      ...
      * @param {*} options
      */
     kenn.sendFileUrl = async (jid, url, caption, quoted, options = {}) => {
      let mime = '';
      let res = await axios.head(url)
      mime = res.headers['content-type']
      if (mime.split("/")[1] === "gif") {
     return kenn.sendMessage(jid, { video: await getBuffer(url), caption: caption, gifPlayback: true, ...options}, { quoted: quoted, ...options})
      }
      let type = mime.split("/")[0]+"Message"
      if(mime === "application/pdf"){
     return kenn.sendMessage(jid, { document: await getBuffer(url), mimetype: 'application/pdf', caption: caption, ...options}, { quoted: quoted, ...options })
      }
      if(mime.split("/")[0] === "image"){
     return kenn.sendMessage(jid, { image: await getBuffer(url), caption: caption, ...options}, { quoted: quoted, ...options})
      }
      if(mime.split("/")[0] === "video"){
     return kenn.sendMessage(jid, { video: await getBuffer(url), caption: caption, mimetype: 'video/mp4', ...options}, { quoted: quoted, ...options })
      }
      if(mime.split("/")[0] === "audio"){
     return kenn.sendMessage(jid, { audio: await getBuffer(url), caption: caption, mimetype: 'audio/mpeg', ...options}, { quoted: quoted, ...options })
      }
      }
    ...
    /** Send List Messaage
      ...
      *@param {*} quoted
      */
        kenn.sendListMsg = (jid, text = '', footer = '', title = '' , butText = '', sects = [], quoted) => {
        let sections = sects
        var listMes = {
        text: text,
        footer: footer,
        ...
        kenn.sendMessage(jid, listMes, { quoted: quoted })
        }
     ...
     * @returns 
     */
        kenn.send5ButMsg = (jid, text = '' , footer = '', but = []) =>{
        ...
        kenn.sendMessage(jid, templateMessage)
        }
     ...
     * @returns
     */
    kenn.send5ButVid = async (jid , text = '' , footer = '', vid, but = [], options = {}) =>{
     ...
     * @returns
     */
    kenn.send5ButGif = async (jid , text = '' , footer = '', gif, but = [], options = {}) =>{
        let message = await prepareWAMessageMedia({ video: gif, gifPlayback: true }, { upload: kenn.waUploadToServer })
        var template = generateWAMessageFromContent(jid, proto.Message.fromObject({
        templateMessage: {
        hydratedTemplate: {
        videoMessage: message.videoMessage,
